...
class IceMenuVTree extends JTree
{
	/**
	...
	{
		$this->_params		= &$params;
		$this->_root		= new IceMenuVNode(1, 'ROOT');
	...
	function addNode(&$params, $item)
	{
		...
		$node = new IceMenuVNode($item->id, $item->title, $item->access, $data);
	...
	function toXML()
	{
		...
		{
			$this->_buffer .= '<div><ul id="iceverticalmenu" class="iceverticalmenu">';
	...
	function _getSubmenuByType()
	{
		...
		switch($submenu_type)
		{
			...
			case "mod":  
				...
				$modHelper = new IceModuleVHelper();
 ...
 * @since		1.5
 */
class IceMenuVNode extends JNode
{
	/**
	 * Node Title
	 */
